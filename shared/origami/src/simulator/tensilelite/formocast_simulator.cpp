...
namespace origami
{
    ...
    static double calculatePrefetchPerformance(int      grvwa,
                                         int      grvwb,
                                         int      bpeA,
                                         int      bpeB,
                                         uint32_t depthU,
    ...
    void Formocast::calculateStorePerformance(double M, double N, double M_WGs_total, double num_tiles, double NumBatches, double MT0, double MT1, uint32_t GWVWD, uint32_t bpeD, const HardwareConstants& hw_consts, uint32_t WGs_per_tile_full, uint32_t WGs_per_tile_XCD_full, double &store, double &store_edge) const
    {
        double D_L1_req = 0.0;
        double D_L2_req = 0.0;
        double D_L3_req = 0.0;
    ...
    Formocast::predictedPerformance(void) const
    {
        ...
        calculateStorePerformance(M, N, M_WGs_total, num_tiles, NumBatches, MT0, MT1, GWVWD, bpeD, hw_consts, WGs_per_tile_full, WGs_per_tile_XCD_full, store, store_edge);
        ...
        double prefetch      = calculatePrefetchPerformance(GRVWA,
                                                 GRVWB,
                                                 bpeA,
                                                 bpeB,
                                                 depthU,
    ...
    Formocast::calculateIntermediateMetrics() const
    {
        ...
        cache_hits.L1_hit = l1;
        cache_hits.L2_hit = l2;
        cache_hits.L3_hit = l3;

        // 5. Calculate Store Performance (D matrix writes)
        double store, store_edge;
        calculateStorePerformance(M, N, M_WGs_total, num_tiles, NumBatches, MT0, MT1, GWVWD, bpeD, hw_consts, WGs_per_tile, WGs_per_tile_XCD, store, store_edge);
        ...
        double tcc_ea0_coalscedA;
        double tcc_ea0_coalscedB;
        double A_L1_req = simulator::getL1LoadRequest(std::min(MT0, M), depthU, hw_consts.L1CacheLineSize, 
                                         GRVWA, bpeA, DTVA, 
                                         transA,           // isTransposed
        ...
        double B_L1_req = simulator::getL1LoadRequest(std::min(MT1, N), depthU, hw_consts.L1CacheLineSize, 
                                         GRVWB, bpeB, DTVB, 
        ...
        double A_L2_req = simulator::getL2LoadRequest(A_L1_req, cache_hits.L1_hit.tile0HitRate, tcc_ea0_coalscedA);
        double A_L3_req = simulator::getL3LoadRequest(A_L2_req, cache_hits.L2_hit.tile0HitRate, tcc_ea0_coalscedA);
        double A_hbm_req = simulator::getHBMLoadRequest(A_L3_req, cache_hits.L3_hit.tile0HitRate);
        double B_L2_req = simulator::getL2LoadRequest(B_L1_req, cache_hits.L1_hit.tile1HitRate, tcc_ea0_coalscedB);
        double B_L3_req = simulator::getL3LoadRequest(B_L2_req, cache_hits.L2_hit.tile1HitRate, tcc_ea0_coalscedB);
        double B_hbm_req = simulator::getHBMLoadRequest(B_L3_req, cache_hits.L3_hit.tile1HitRate);
    ...
    Formocast::calculateFinalPerformance(const IntermediatePerformanceMetrics& metrics) const
    {
        ...
        uint32_t N_WGs_total = safe_ceil_div(static_cast<uint32_t>(N), static_cast<uint32_t>(MT1));
        int N_WGs_per_tile_XCD = std::min((uint32_t)WGM, N_WGs_total);
        int M_WGs_per_tile_XCD = std::min(M_WGs_total, static_cast<uint32_t>(safe_ceil_div(int(hw_consts.NumCUs / hw_consts.NumXCDs), N_WGs_per_tile_XCD)));
        int M_WGs_per_tile = std::min(M_WGs_total, static_cast<uint32_t>(safe_ceil_div(int(hw_consts.NumCUs), N_WGs_per_tile_XCD)));
        int N_WGs_per_tile = std::min(N_WGs_total, static_cast<uint32_t>(N_WGs_per_tile_XCD * safe_ceil_div(M_WGs_per_tile, M_WGs_total)));
        uint32_t numberWGs = M_WGs_total * N_WGs_total * NumBatches * GlobalSplitU;
        uint32_t WGs_per_tile = std::min(uint32_t(hw_consts.NumCUs), numberWGs);
        uint32_t WGs_per_tile_XCD = safe_ceil_div(WGs_per_tile, hw_consts.NumXCDs);
        uint32_t num_tiles = safe_ceil_div(numberWGs, uint32_t(hw_consts.NumCUs));
        ...
        // 1. Calculate Memory Access Costs using new integrated function
        MemoryAccessCosts mem_costs = calculateMTMemoryAccessCosts(M, N, K_AfterGSU,
                                                                MT0, MT1,
                                                                hw_consts,
