...
def reachable(prog):
    ...
    Reachability, not dominance: some execution may run one block before another even where the
    first does not dominate the second."""
...
class BackEdge:
 ...
 GenXfer list so frame_map keys the transfer per back-edge (not xfers[0])."""
