...
def _counter_sources(prog, edges):
    return frozenset((counter_for(edge, prog), id(edge.producer.inst)) for edge in edges)


def _one_barrier(prog, covers, n_slots, block):
    """Merge covers of one producer when moving crosses no same-counter issue."""
    merged = []
    ...
    for edges in covers:
        ...
        own_slot = max(legal)
        for i, (slot, have, kept) in enumerate(merged):
            common = max(have & legal) if have & legal else None
            counters = {counter for counter, _inst in _counter_sources(prog, edges)}
            no_loss = (common is not None
                       and _counter_sources(prog, kept) == _counter_sources(prog, edges)
                       and all(issued(body, common, own_slot, counter, counts) == 0
                               and issued(body, common, slot, counter, counts) == 0
                               for counter in counters))
            if no_loss:
                merged[i] = (common, have & legal, kept + list(edges))
                break
        else:
            merged.append((own_slot, legal, list(edges)))
    return [edges for _slot, _slots, edges in merged]
...
class FenceRegions(Analysis):
    ...
    def run(self, prog, am):
        ...
        for blk in prog.walk_rpo():
            ...
            taken = set()
            for covered in (_one_barrier(prog, covers, n_slots, blk.label)
                            if self.merge else covers):
        ...
        return [PendingMark(
            ...
            region=_at_slot(prog, blk.label, _slot),
