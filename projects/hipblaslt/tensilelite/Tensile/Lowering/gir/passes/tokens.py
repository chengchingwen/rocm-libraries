...
"""Attach frame-relative fence relations and the legacy backend token compatibility stamps.

The relation is authoritative: `(frame + gdelta) % ring` resolves each concrete token. Static
dependency-token unions remain only until the downstream scheduler consumes that relation.
"""
...
def _frame_access(touch, frame, fm, storage):
    ref = touch.ref
    ring = max(1, int(storage.depth_of(ref.tile.operand)))
    absolute = getattr(ref, "abs_gen", None) is not None
    gdelta = int(ref.abs_gen) if absolute else int(getattr(ref, "gdelta", 0) or 0)
    return {
        "at": f"{touch.block}[{touch.pos}]",
        "identity": id(touch.inst),
        "operand": touch.operand,
        "space": ref.tile.space,
        "frame": fm.render(frame),
        "ring": ring,
        "gdelta": gdelta,
        "absolute": absolute,
        "token": fm.generation(ref, frame),
        "regions": tuple(tuple(sorted(values)) for values in getattr(touch, "regions", ())),
        "advance": int(getattr(touch.inst, "advance", 0) or 0),
    }


def _fence_relations(pm, hazards, fm, storage):
    def key(hazard):
        return (hazard.kind, hazard.producer.block, hazard.producer.pos,
                id(hazard.producer.ref), hazard.consumer.block, hazard.consumer.pos,
                id(hazard.consumer.ref), int(hazard.gap), bool(hazard.cross_agent))

    covered = {key(hazard) for hazard in pm.edges}
    return tuple({
        "kind": hazard.kind,
        "gap": int(hazard.gap),
        "scope": "block" if hazard.cross_agent else "wave",
        "producer": _frame_access(hazard.producer, fp, fm, storage),
        "consumer": _frame_access(hazard.consumer, fc, fm, storage),
    } for fp, fc, hazard in hazards.instances() if key(hazard) in covered)
    ...
    def run(self, prog, am):
        ...
        for pm in prog.pending:
            ...
            pm.mark.at = dict(pm.mark.at or {},
                              relations=_fence_relations(pm, hazards, fm, storage),
                              tokens=(), order_tokens=(), no_waitcnt=True)
            changed = True
