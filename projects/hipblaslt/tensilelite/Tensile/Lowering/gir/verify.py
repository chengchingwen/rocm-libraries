...
from .analyses.fence_regions import fences_of, separated
...
def _check_frame_coverage(prog, dep, hz, am):
    ...
    Either its two ends share a token -- the rotation relabels both together, so one frame's answer
    is every frame's -- or a fence stands between them.  A fence naming nothing is a full drain and
    counts; it is over-draining, not a hole.
...
def _check_token_reuse(prog, tokens, dep, hz):
    """G-TOKEN-REUSE: every cross-wave edge in the FINAL body is separated by a naming fence.

    `FenceRegions` asserts this of the cover it plans; this re-derives it from the marks that
    actually landed, so a placement or an apply that lost one is caught rather than assumed."""
    fences = fences_of(prog)
    n_of = {lab: len(b.body) + 1 for lab, b in prog.blocks.items()}
    for h in hz.needing_fence():
        # ONLY A RAW IS STAMPED, by its CONSUMER: the fence is there for the reads after it.
        # Same rule the naming uses -- derived from the refs, never from the fence, or the check
        # cannot fail.
        pins = set(dep.tokens_for(h.consumer.ref)) if h.kind == RAW else set()
        # A WRAP shares no token by construction -- its ends run one rotation apart, so no frame
        # reads them together -- and is discharged by the fence POSITION in the producing trip.
        if not pins or separated(h, fences, n_of.get, pins):
            continue
        raise RuntimeError(
            f"G-TOKEN-REUSE: the {h.kind} edge {h.producer.block}[{h.producer.pos}] -> "
            f"{h.consumer.block}[{h.consumer.pos}] races across waves and no fence between its "
            f"ends names any of its tokens {sorted(pins)} -- the barrier orders every buffer "
            f"except the one at risk")
...
def check_block_scope_covered(prog, am):
    ...
    `FrameHazards`' buffer overlap across waves -- must agree, or a cooperative fill and the reads
