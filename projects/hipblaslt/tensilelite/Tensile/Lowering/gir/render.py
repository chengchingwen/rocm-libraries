...
def _region_fact(m, prog):
    ...
    if regions:
        ...
        for op in sorted(regions):
            ...
            #
            who = ("address is wave-chosen on the read hop, while the coordinate still selects "
                   "one independent LDS generation/token"
...
def render_gir(prog) -> str:
    ...
    for blk in prog.walk_rpo():
        ...
        L.append(head + ":")
        for phi in blk.phis:
            L.append(f"  phi:  {_gen_name(prog, phi.gen)} = "
                     f"phi(entry:{phi.entry_val}, back) ring={phi.gen.ring}")
