...
class GirToRocisa:
    ...
    def emit_block(self, prog, phase, *, tpByOperand=None, internalPointerSwap=False) -> Module:
        ...
        for _i, act in enumerate(acts):
            ...
            self._stamp_action(out.flatitems()[before:], act.action_id)
        return out

    def _stamp_action(self, items, action_id):
        """Attach the stable semantic action id to every physical realization."""
        for item in items:
            setter = getattr(item, "setGirActionData", None)
            if not callable(setter):
                continue
            setter(int(action_id))
