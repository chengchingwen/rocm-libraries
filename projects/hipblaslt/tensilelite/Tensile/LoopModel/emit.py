...
class _Emitter:
    ...
    def __init__(self, theta, depths, ledger=None, plans=None):
        ...
        def copy_priority(operand):
            ...
            readers = [copy_by_name[name] for name in unit if name in copy_by_name]
            return min((self._read_issue_priority(member) for member in readers),
                       default=(len(self.pos), 0, 1, self._operand_order[operand.name]))
    ...
    def _read_issue_priority(self, operand, groups=None, ceiling=None):
        """Loop site, prefetch depth, register width, declaration order."""
        ...
        width = min(max(1, self.S.get(operand.name, group)) for group in selected)
        return level, depth, width, self._operand_order[operand.name]
    ...
    def _first_touch_axes(self, operand):
        ...
        retains = all(max(1, self.S.get(operand.name, group))
                      >= geometry.group_reuse_floor(self.theta, operand, group)
    ...
    def fill_reads(self):
        ...
        for operand in self.reads:
            for ordinal, coord in enumerate(preloaded_tiles(
                    self.theta, operand, self.plans.want(operand), self.S, self.plans)):
    ...
    def _guarded_read(self, operand, groups, ceiling, suppress_ahead):
        ...
        selected = tuple(groups or operand.fragment.groups())
        if (geometry.reloads_whole_set(self.theta, operand)
                or operand.name.startswith("MXS")):
            reload_cycle = max(1, span)
        else:
            width = min(max(1, self.S.get(operand.name, group)) for group in selected)
            reload_cycle = max(
                1,
        ...
        if not suppress_ahead and remainder and invariant:
            def guarded(body, values):
                for guard_axis, step in reversed(first_touch):
                    lhs = Expr(var=guard_axis, mod=step) if step > 1 else Expr(var=guard_axis)
                    body = Cond(pred=Pred(lhs, "==", values.get(guard_axis, 0)),
                                then=[body], els=[], kind="first_touch")
                return body

            full_extents = {axis.name: axis.extent for axis in self.theta.inner_axes()}
            late = {axis: max(0, int(full_extents.get(axis, 1)) - 1)
                    for axis, _step in invariant}
            nowrap = Pred(Expr(terms=position_terms(strides, coverage), mod=reload_cycle),
                          "<", reload_cycle - remainder)
            return Cond(pred=nowrap, then=[guarded(node, {})],
                        els=[guarded(node, late)], kind="reload_wrap"), True
        if not suppress_ahead and advance >= reload_cycle and not remainder and invariant:
            full_extents = {axis.name: axis.extent for axis in self.theta.inner_axes()}
            anchor = {axis: max(0, int(full_extents.get(axis, 1)) - 1)
                      for axis, _step in invariant}
        else:
