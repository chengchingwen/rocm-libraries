...
def _geometry_ring_table(lines, depths, requested_steps, theta):
    ...
    lines.append("  #   W_reg = register-ring width per group; every group shares its operand PLR")
...
def render_geometry(theta) -> str:
    ...
    lines.append("  #   region_count contains region factors inside this rotation unit; an outer "
             "reduction region rotates through the same names and does not multiply the footprint.")
