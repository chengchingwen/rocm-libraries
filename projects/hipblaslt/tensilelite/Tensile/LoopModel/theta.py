...
class Fragment:
    ...
    grouping_mode: str = None  # first factor of the logical axis whose unit values groups partition
    group_policy: dict = None  # label -> 'unroll'|'overlap'|'inplace'|int W; "*"=default
    #: hardware read instructions ONE fill of this placement issues.  Required for a register
    #: placement -- a completion counter counts instructions, so 0 makes every wait over it a drain.
    instructions: int = 0
...
class Operand:
    ...
    free_split: int = 1
    #: Physical regions touched by one wave's read addresses.  This affects region aliasing, not
    #: the split-axis extent; the coordinate identity is free_split * free-inner = MIWaveTile.
