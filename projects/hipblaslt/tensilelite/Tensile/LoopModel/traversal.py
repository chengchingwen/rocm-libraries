...
def _varying_modes(theta, operand):
    ...
    A read quantum changes how many instructions move those tiles; it does not change the
    prefetch or rotation unit that owns them.  Keep the physical extents here and apply the
    quantum only when naming a covered register position.
    """
...
def global_outer_role(theta):
    """Outermost logical role in ``ord``, including a TDMSplit region factor."""
    return next((mode_role(axis.name) for axis in theta.inner_axes()
                 if mode_role(axis.name) is not None), None)


def operand_outer_role(theta, operand):
    """Outermost logical role this operand varies on."""
    return next((mode_role(name) for name, _extent in _varying_modes(theta, operand)
...
def operand_inner_role(theta, operand):
    ...
    roles = [mode_role(name) for name, _extent in _varying_modes(theta, operand)
...
def unit_tile_index(theta, operand, coord) -> int:
    ...
    group_extent = grouping_extent(theta, operand)
    group_extent = max(1, group_extent // groups)

    modes = []
    # A region factor that belongs to this rotation unit is packed inside its (group, slot) block.
    # A reduction region outside the unit is an outer enumerator and reuses the same names.
    inner = {axis.name: axis.extent for axis in theta.inner_axes()}
    reduction = set(summation_names(theta))
    rotation_regions = {name for name, _extent in rotation_unit_modes(theta, operand)}
    for name in getattr(operand, "region_axes", ()) or ():
        if name in inner and name in rotation_regions:
            span = max(1, int((getattr(operand, "region_span", {}) or {}).get(name, 1)))
            modes.append((name, max(1, inner[name] // span), span, False))
    for name, extent in rotation_unit_modes(theta, operand):
        if name in (getattr(operand, "region_axes", ()) or ()):
            continue
        modes.append((name, group_extent if name == grouping else extent, 1,
                      name == grouping))
    represented = {name for name, _extent, _divisor, _is_grouping in modes}
    regions = set(getattr(operand, "region_axes", ()) or ())
    ...
    for name, extent in prefetch_unit_modes(theta, operand):
        if name in represented or name in regions or name in ring_names:
            continue
        modes.append((name, group_extent if name == grouping else extent, 1,
                      name == grouping))
    ...
    def flatten(selected):
        value, total = 0, 1
        for name, extent, divisor, is_grouping in selected:
            raw = int(at.get(name, 0) or 0)
            if is_grouping:
                raw %= max(1, grouping_extent(theta, operand) // groups)
            digit = raw // divisor
    ...
    hop = _register_read_hop(operand)
    quantum = getattr(hop, "quantum", None) if hop is not None else None
    if quantum is None:
        return flatten(modes)[0]
    if operand.name.startswith("MXS"):
        regions = set(getattr(operand, "region_axes", ()) or ())
        logical = []
        for axis in presence_axes(theta, operand):
            name, extent = axis.name, axis.extent
            if name in regions:
                span = max(1, int((getattr(operand, "region_span", {}) or {}).get(name, 1)))
                logical.append((name, max(1, extent // span), span, False))
            else:
                logical.append((name, group_extent if name == grouping else extent, 1,
                                name == grouping))
        free_modes = [mode for mode in logical if mode[0] not in reduction]
        reduction_modes = [mode for mode in logical if mode[0] in reduction]
        free_value, free_total = flatten(free_modes)
        reduction_value, _reduction_total = flatten(reduction_modes)
        pairs = [(quantum.carrier_of(tile), quantum.slot_of(tile))
                 for tile in range(max(1, free_total))]
        order = []
        for pair in pairs:
            if pair not in order:
                order.append(pair)
        free_index = order.index(pairs[free_value])
        return reduction_value * len(order) + free_index
    free_modes = [mode for mode in modes if mode[0] not in reduction]
    reduction_modes = [mode for mode in modes if mode[0] in reduction]
    free_value, free_total = flatten(free_modes)
    reduction_value, reduction_total = flatten(reduction_modes)
    free_index = free_value
    carriers = [quantum.carrier_of(tile) for tile in range(max(1, free_total))]
    order = []
    for carrier in carriers:
        if carrier not in order:
            order.append(carrier)
    free_index = order.index(carriers[free_value])
...
def _covered_positions(theta, operand, modes, grouped=False) -> int:
    ...
    hop = _register_read_hop(operand)
    factors = (coverage_factors(theta, operand, getattr(hop, "quantum", None))
...
def grouping_preserves_quantum(theta, operand, groups) -> bool:
    ...
    for tile, values in enumerate(_iproduct(*ranges)):
        ...
        carrier = quantum.carrier_of(tile)
...
def group_reuse_floor(theta, operand, group) -> int:
    ...
    hop = _register_read_hop(operand)
    if hop is not None and hop.quantum is not None:
        cache[cache_key] = 1
        return 1
...
def requested_read_ahead(theta, operand, depths=None) -> int:
    ...
    if reloads_whole_set(theta, operand) and not has_split_region:
        ...
        original = prefetch_unit_tiles(theta, operand)
...
def _staged_prefetch_cap(theta, operand, want) -> int:
    ...
    outer = theta.summation_chunk_name()
    if outer is None or not any(hop.dst == Space.SHARED for hop in operand.hops):
...
def group_register_positions(theta, operand) -> int:
    """Physical register positions in one (group, rotation-slot) block."""
    ...
    return max(1, len({(quantum.carrier_of(tile), quantum.slot_of(tile))
...
def operand_footprint_regs(theta, operand, depths) -> int:
    ...
    total = sum(depths.get(operand.name, group) * positions
                for group in operand.fragment.groups())
...
def register_layout(theta, depths) -> dict:
    ...
    layout = {}
    for operand in theta.operands:
        if operand.is_output or not operand.hops:
            continue
        offset, slots = 0, {}
        ...
        for group_index, group in enumerate(operand.fragment.groups()):
            ...
            for slot in range(max(1, int(depths.get(operand.name, group)))):
...
def ring_axes(theta, operand, group):
    ...
    hop = _register_read_hop(operand)
    if operand.name.startswith("MXS") and hop is not None and hop.quantum is not None:
        summation = set(summation_names(theta))
        return [(name, extent) for name, extent in _varying_modes(theta, operand)
