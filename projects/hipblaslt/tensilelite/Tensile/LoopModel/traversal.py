...
def operand_emitted_regs(theta, operand, depths) -> int:
    """VGPRs reserved by the current ``Valu*_X*`` physical layout.

    The scaffold gives every X slot one full, buffer-major physical operand block.  Register
    groups may use only part of a slot, but they do not yet have separately packed bases, so the
    allocation must cover the widest group across that complete block.
    """
    if operand.is_output:
    ...
    fan = free_tiles(theta, operand)
    hop = _register_read_hop(operand)
    quantum = getattr(hop, "quantum", None)
    if quantum is not None:
        # Distinct (carrier, slot) pairs are the physical register positions.  A contiguous
        # vector load has several slots per carrier and keeps the full fan; a distributed MXS
        # TileSpan maps partner tiles to the same slot and therefore halves the physical block.
        fan = len({(quantum.carrier_of(tile), quantum.slot_of(tile)) for tile in range(fan)})
    return max(1, max(widths)) * max(1, fan) * frag_regs(theta, operand)


def _region_count(theta, operand) -> int:
    """Free-axis storage regions replicated in this operand's register footprint."""
    summation = set(summation_names(theta))
