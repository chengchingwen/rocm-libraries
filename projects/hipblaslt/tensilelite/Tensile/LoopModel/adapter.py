...
class _SBuilder:
    ...
    The scheme is PER OPERAND, not one choice imposed on all of them.  Operands are visited
    most-important first -- A/B before their microscale rings, and within each pair the one whose
    read-ahead covers its whole set (B under MKN) before the one that steps -- and each takes the
    earliest scheme it can carry.  Whether an operand carries a scheme depends only on its own
    fragment, so the registers are the single shared resource, and they are what makes the search
    back up to the previous operand instead of running left to right.
    """
    ...
    def order(self, depths) -> list:
        """The operands, most important first.

        A/B outrank their microscale rings, and within each rank the operand whose read-ahead
        covers its WHOLE SET -- B under MKN, A under NKM -- is placed before the one that steps
        through its fan, because its ring is the one a loop order leaves no slack in.  Neither is
        whole-set under a K-outermost order, so equals are ordered SMALLEST FIRST: the cheap ring
        commits few registers before the costly one chooses.
        """
    ...
    def cost(self, depths, operands=None) -> int:
        """Registers the current physical ``Valu*_X*`` layout reserves for this S."""
